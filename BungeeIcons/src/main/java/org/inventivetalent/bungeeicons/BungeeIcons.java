...
public class BungeeIcons extends Plugin implements Listener {
    ...
    void reloadIcons() {
        ...
        List<Map<String, String>> icons = (List<Map<String, String>>)config.getList("icons", null);
        if (icons == null) {
            getLogger().info("No icons configured");
            return;
        }
        getLogger().info("Loading " + icons.size() + " icons...");
        for (Map<String, String> map : icons) {
            String key = map.get("host");
            String value = map.get("icon");
            ...
            if (iconFile.exists()) {
                try {
                    ...
                    getLogger().log(Level.WARNING, "Failed to read image " + iconFile + " (" + key + ")", e);
                }
